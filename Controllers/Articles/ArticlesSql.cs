...
{
    public static class ArticlesSql
    {
        public static SiteBackEnd.Models.Home.HomeBoard[] GetBoard(int i) {
            var conn = Controllers.SQL.CreateConnection();
            var command = new NpgsqlCommand("SELECT \"Name\", \"Description\", \"photoPath\",\"RealName\" FROM schema.\"Articles\" ORDER BY \"dateAdded\" DESC LIMIT 5 OFFSET " + i*5, conn);
            try
            {
                conn.Open();
            }
        ...
        {
            var conn = Controllers.SQL.CreateConnection();
            var command = new NpgsqlCommand("SELECT COUNT(\"ID\") FROM schema.\"Articles\"", conn);
        ...
        public static Models.Home.QueryModel.PathModel GetPath(string str) {
            var conn = Controllers.SQL.CreateConnection();
            var command = new NpgsqlCommand("SELECT \"RealName\", \"Path\" FROM schema.\"Articles\" WHERE \"RealName\" = \'" + str + "\'", conn);
            try
            {
                conn.Open();
            }
